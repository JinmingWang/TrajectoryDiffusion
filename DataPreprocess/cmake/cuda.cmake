# ---[ cuda
set(CUDA_INCLUDE_DIRS "/usr/local/cuda-12/include")
set(CUDA_CUDART_LIBRARY "/usr/local/cuda-12/lib64/libcudart.so")
set(CUDA_NVCC_EXECUTABLE "/usr/local/cuda-12/bin/nvcc")
set(CUDA_TOOLKIT_INCLUDE "/usr/local/cuda-12/include")
set(CMAKE_CUDA_HOST_COMPILER "${CMAKE_CXX_COMPILER}")
set(CMAKE_CUDA_FLAGS "${CMAKE_CUDA_FLAGS} -ccbin ${CMAKE_CXX_COMPILER}")

# Poor man's include guard
if(TARGET preprocess::cudart)
  return()
endif()
...
if(NOT CUDA_FOUND)
  message(WARNING
    "preprocess: CUDA cannot be found. Depending on whether you are building "
    "preprocess or a preprocess dependent library, the next warning / error will "
    "give you more info.")
  set(preprocess_USE_CUDA OFF)
  return()
endif()
message(STATUS "preprocess: CUDA detected: " ${CUDA_VERSION})
message(STATUS "preprocess: CUDA nvcc is: " ${CUDA_NVCC_EXECUTABLE})
message(STATUS "preprocess: CUDA toolkit directory: " ${CUDA_TOOLKIT_ROOT_DIR})
...
if(CUDA_FOUND)
  ...
  if(NOT compile_result)
    message(FATAL_ERROR "preprocess: Couldn't determine version from header: " ${output_var})
  endif()
  message(STATUS "preprocess: Header version is: " ${cuda_version_from_header})
...
# Find cuDNN.
if(preprocess_STATIC_LINK_CUDA AND NOT USE_STATIC_CUDNN)
  message(WARNING "cuDNN will be linked statically because preprocess_STATIC_LINK_CUDA is ON. "
    "Set USE_STATIC_CUDNN to ON to suppress this warning.")
endif()
if(preprocess_STATIC_LINK_CUDA OR USE_STATIC_CUDNN)
...
if(NOT CUDNN_FOUND)
  message(WARNING
    "preprocess: Cannot find cuDNN library. Turning the option off")
  set(preprocess_USE_CUDNN OFF)
else()
  set(preprocess_USE_CUDNN ON)
endif()

# Optionally, find TensorRT
if(preprocess_USE_TENSORRT)
  ...
  if(NOT TENSORRT_FOUND)
    message(WARNING
      "preprocess: Cannot find TensorRT library. Turning the option off")
    set(preprocess_USE_TENSORRT OFF)
  endif()
endif()

# ---[ Extract versions
if(preprocess_USE_CUDNN)
  # Get cuDNN version
...
# Several of these libraries have a hardcoded path if preprocess_STATIC_LINK_CUDA
...
# cuda
add_library(preprocess::cuda UNKNOWN IMPORTED)
set_property(
    TARGET preprocess::cuda PROPERTY IMPORTED_LOCATION
    ${CUDA_CUDA_LIB})
set_property(
    TARGET preprocess::cuda PROPERTY INTERFACE_INCLUDE_DIRECTORIES
    ${CUDA_INCLUDE_DIRS})

# cudart. CUDA_LIBRARIES is actually a list, so we will make an interface
# library.
add_library(preprocess::cudart INTERFACE IMPORTED)
if(preprocess_STATIC_LINK_CUDA)
    set_property(
        TARGET preprocess::cudart PROPERTY INTERFACE_LINK_LIBRARIES
...
set_property(
    TARGET preprocess::cudart PROPERTY INTERFACE_INCLUDE_DIRECTORIES
    ${CUDA_INCLUDE_DIRS})

# cudnn
# static linking is handled by USE_STATIC_CUDNN environment variable
if(preprocess_USE_CUDNN)
  add_library(preprocess::cudnn UNKNOWN IMPORTED)
  set_property(
      TARGET preprocess::cudnn PROPERTY IMPORTED_LOCATION
      ${CUDNN_LIBRARY_PATH})
  set_property(
      TARGET preprocess::cudnn PROPERTY INTERFACE_INCLUDE_DIRECTORIES
      ${CUDNN_INCLUDE_PATH})
endif()

# curand
add_library(preprocess::curand UNKNOWN IMPORTED)
if(preprocess_STATIC_LINK_CUDA)
    set_property(
        TARGET preprocess::curand PROPERTY IMPORTED_LOCATION
...
set_property(
    TARGET preprocess::curand PROPERTY INTERFACE_INCLUDE_DIRECTORIES
    ${CUDA_INCLUDE_DIRS})

# cufft. CUDA_CUFFT_LIBRARIES is actually a list, so we will make an
# interface library similar to cudart.
add_library(preprocess::cufft INTERFACE IMPORTED)
if(preprocess_STATIC_LINK_CUDA)
    set_property(
        TARGET preprocess::cufft PROPERTY INTERFACE_LINK_LIBRARIES
...
set_property(
    TARGET preprocess::cufft PROPERTY INTERFACE_INCLUDE_DIRECTORIES
    ${CUDA_INCLUDE_DIRS})

# TensorRT
if(preprocess_USE_TENSORRT)
  add_library(preprocess::tensorrt UNKNOWN IMPORTED)
  set_property(
      TARGET preprocess::tensorrt PROPERTY IMPORTED_LOCATION
      ${TENSORRT_LIBRARY})
  set_property(
      TARGET preprocess::tensorrt PROPERTY INTERFACE_INCLUDE_DIRECTORIES
      ${TENSORRT_INCLUDE_DIR})
endif()

# cublas. CUDA_CUBLAS_LIBRARIES is actually a list, so we will make an
# interface library similar to cudart.
add_library(preprocess::cublas INTERFACE IMPORTED)
if(preprocess_STATIC_LINK_CUDA)
    set_property(
        TARGET preprocess::cublas PROPERTY INTERFACE_LINK_LIBRARIES
        "${CUDA_TOOLKIT_ROOT_DIR}/lib64/libcublas_static.a")
    if (CUDA_VERSION VERSION_EQUAL 10.1)
      set_property(
        TARGET preprocess::cublas APPEND PROPERTY INTERFACE_LINK_LIBRARIES
...
set_property(
    TARGET preprocess::cublas PROPERTY INTERFACE_INCLUDE_DIRECTORIES
    ${CUDA_INCLUDE_DIRS})

# nvrtc
add_library(preprocess::nvrtc UNKNOWN IMPORTED)
set_property(
    TARGET preprocess::nvrtc PROPERTY IMPORTED_LOCATION
    ${CUDA_NVRTC_LIB})
set_property(
    TARGET preprocess::nvrtc PROPERTY INTERFACE_INCLUDE_DIRECTORIES
    ${CUDA_INCLUDE_DIRS})

# Note: in theory, we can add similar dependent library wrappers. For
# now, preprocess only uses the above libraries, so we will only wrap
# these.
...
if (MSVC)
  if (${preprocess_USE_MSVC_STATIC_RUNTIME})
